...
def CostFunction(x):
    ...
    # =====================运行成本======================
    Owt_unit = 0.03  # 风机单位运行成本
    Opv_unit = 0.02  # 光伏单位运行成本
    Obat_unit = 0.018  # 电池运行成本
    Opgu_unit = 0.05  # 燃气轮机单位运行成本
    Oac_unit = 0.048  # 吸收式制冷机单位运行成本
    Oec_unit = 0.06  # 电制冷机单位运行成本
    Ogb_unit = 0.04  # 燃气锅炉运行成本
    Oeb_unit = 0.08  # 电锅炉单位运行成本
    Ohss_unit = 0.016  # 蓄热槽运行成本
...
这种以冷定热、以热定电的耦合关系可以实现能源的高效利用，提高系统的能源利用率，减少能源浪费。
    ...
    # ========冷子系统==========
        ...
        if Hpgu[t] < Hload[t]: #燃气轮机不够满足热需求
            ...
            if Hhot2[t] <= Hhss_out_max: #首先考虑蓄热槽放热
                if (0.7855 * Hsocmin <= Hsoc[t]) and (Hsoc[t] <= Hsocmax):
                    ...
                    Hsoc[t + 1] = Hsoc[t] * (1 - e_loss) - (Hhss_out[t] / e_hss_out) / Uhss     #%%%%%%%revised理解充放热功率与效率的关系
                ...
                if (0.7855 * Hsocmin <= Hsoc[t]) and (Hsoc[t] <= Hsocmax):
                    ...
                    Hhss_out[t] = 0
                    Hsoc[t + 1] = Hsoc[t] * (1 - e_loss) - (Hhss_out[t] / e_hss_out) / Uhss #%%%%%%%revised理解充放热功率与效率的关系 下同
                    Hgb_h[t] = Hhot2[t] - Hhss_out[t]
        ...
        Pnet[t] = Pwt[t] + Ppv[t] - Pload[t] - Pec[t] + Ppgu[t] - Peb[t]    #-Pehac(t); %电网净负荷 + 考虑多余的热转电Hche(t) * e_che, 这里燃气轮机耗电
        b_loss = 0.0001#自放电率
        SOC[1] = 0.4
        #=========================充电==============================
    ...
    TotalPsell = sum(Psell)
    TotalPbuy = sum(Pbuy)
    TotalCostPsell = sum(CostPsell)
    TotalCostPbuy = sum(CostPbuy)
    #========================成本计算==========================
    # 成本计算
    #======================运行成本定义=========================
    ...
    Cvpgu = np.zeros(8760)  # 燃气轮机消耗天然气成本
    Cvgb = np.zeros(8760)  # 燃气锅炉消耗天然气成本
    ...
    #======================投资成本============================
    Cinv_wt = Fcr * Uwt * Cwt_unit
    Cinv_pv = Fcr * Upv * Cpv_unit
    Cinv_bat = Fcr * Ubat * Cbat_unit
    Cinv_pgu = Fcr * Upgu * Cpgu_unit
    Cinv_ac = Fcr * Uac * Cac_unit
    Cinv_ec = Fcr * Uec * Cec_unit
    Cinv_gb = Fcr * Ugb * Cgb_unit
    Cinv_eb = Fcr * Ueb * Ceb_unit
    Cinv_hss = Fcr * Uhss * Chss_unit
    #=======================置换成本==========================
    Cre_wt = Frk_20 * Fcr * Uwt * Cwt_unit  # 风机置换成本
    Cre_pv = Frk_20 * Fcr * Uwt * Cpv_unit  # 光伏置换成本
    Cre_bat = Frk_10 * Fcr * Ubat * Cbat_unit  # 蓄电池置换成本
    Cre_ac = Frk_20 * Fcr * Uac * Cac_unit  # 吸收式制冷机置换成本
    Cre_ec = Frk_20 * Fcr * Uec * Cec_unit  # 电制冷机置换成本
    Cre_pgu = Frk_20 * Fcr * Upgu * Cpgu_unit  # 燃气轮机机置换成本
    Cre_gb = Frk_20 * Fcr * Ugb * Cgb_unit  # 燃气锅炉机置换成本
    Cre_eb = Frk_20 * Fcr * Ueb * Ceb_unit  #
    Cre_hss = Frk_10 * Fcr * Uhss * Chss_unit  # 蓄热槽置换成本
    #==================相关成本计算=================
    for t in range(8760):
        ComPwt[t] = Owt_unit * Pwt[t]  # 风机运行成本
        ComPpv[t] = Opv_unit * Ppv[t]  # 光伏运行成本
        ComQpgu[t] = Opgu_unit * Qpgu[t]  # 燃气轮机运行成本
        ComCac[t] = Oac_unit * Cac[t]  # 吸收式制冷机运行成本
        ComCec[t] = Oec_unit * Cec[t]  # 电制冷机运行成本
        ComHeb[t] = Oeb_unit * Heb[t]  # 电锅炉运行成本
        ComHgb[t] = Ogb_unit * Hgb[t]  # 燃气锅炉机运行成本
        ComPcha[t] = Obat_unit * Pcha[t]  # 蓄电池运行成本
        ComPdischa[t] = Obat_unit * Pdischa[t]  # 蓄电池运行成本
        Cvpgu[t] = Cgas * Vpgu[t]  # 燃气轮机消耗天然气成本
        Cvgb[t] = Cgas * Vgb[t]  # 燃气锅炉消耗天然气成本
        ComHhss_out[t] = Opgu_unit * Hhss_out[t]  # 蓄热槽运行成本
        ComHhss_in[t] = Opgu_unit * Hhss_in[t]  # 蓄热槽运行成本
    ...
    TotalEmpgu = sum(Empgu)
    TotalEmgb = sum(Emgb)
    TotalEmGrid = sum(EmGrid)

    CostVpgu = np.zeros(8760)  # 燃气轮机购气总成本
    CostVgb = np.zeros(8760)  # ？// 燃气锅炉购气总成本

    TotalCostVpgu = sum(Cvpgu)  # 燃气轮机购气总成本
    TotalCostVgb = sum(Cvgb)  # 燃气锅炉购气总成本
    TotalCostGas = TotalCostVgb + TotalCostVpgu
    TotalEmco2 = sum(Emco2)  # 二氧化碳总排放量
    TotalPload = sum(Pload)
    TotalPec = sum(Pec)

    #=======================目标函数计算===========================
    Cre = Cre_bat + Cre_hss
    Cinv = Cinv_wt + Cinv_pv + Cinv_bat + Cinv_pgu + Cinv_ac + Cinv_ec + Cinv_gb + Cinv_eb + Cinv_hss
    ...
    ATC = Cinv + Com + Cre#目标函数1  #年成本：廉价-
    CEV = TotalEmco2#目标函数2  # 碳排放：第二昂贵
...
'''x=[1.059121754017121e+02,1.756468605120355e+02,1.089371238687390e+02,1.151439002401878e+02,2.231480832522777e+02,3.062942184094881e+02,2.574042960618399e+02,10]
F = CostFunction(x)
print(F)'''
